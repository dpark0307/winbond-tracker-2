...
def _mouser_qty1_price(price_breaks):
    """Same principle as _qty1_price for DigiKey: find the price break
    specifically at Quantity == 1, not just the minimum across every
    quantity tier (which would wrongly grab a bulk-discount price)."""
    cleaned_breaks = []
    for pb in price_breaks:
        ...
        if not cleaned:
            continue
        try:
            cleaned_breaks.append((pb.get("Quantity", 0), float(cleaned)))
        except ValueError:
            continue
    ...
    qty1 = [price for qty, price in cleaned_breaks if qty == 1]
    if qty1:
        return qty1[0]
    # Fallback: no explicit qty-1 break, use the lowest-quantity break
    cleaned_breaks.sort(key=lambda x: x[0])
    return cleaned_breaks[0][1]


def fetch_mouser_price(part_number, api_key):
    """
    Same two bug classes fixed here as in fetch_digikey_price:

    1. Exact string matching on ManufacturerPartNumber was too strict -
       normalized now (strip spaces/hyphens, uppercase) so trivial
       formatting differences don't wrongly exclude the correct listing.

    2. Previously took min() across ALL price breaks (every quantity
       tier), which would wrongly grab a bulk/high-quantity discount
       price instead of the real qty-1 price. Fixed to explicitly find
       the Quantity == 1 break.
    """
    ...
    for part in parts:
        mfr_pn = _normalize_part(part.get("ManufacturerPartNumber"))
        if mfr_pn != target:
            continue

        price = _mouser_qty1_price(part.get("PriceBreaks") or [])
        if price is not None:
...
def load_last_prices():
    ...
    recent saved price. Keyed by source too, since the same part
    number can have a separate DigiKey price and Mouser price - they
    must be compared independently, not against each other. Skips
    any malformed/incomplete rows instead of crashing the whole script.
    """
    last = {}
    ...
    with open(CSV_PATH, newline="") as f:
        for row in csv.DictReader(f):
            ...
            source = row.get("source") or "DigiKey"  # older rows predate this column
...
def main():
    ...
    if not digikey_id or not digikey_secret:
        ...
        sys.exit(1)
    if not mouser_key:
        print("ERROR: MOUSER_API_KEY is not set.")
        sys.exit(1)
    ...
    print(f"\nPulling prices for {len(active_parts)} part(s) x 2 sources on {today}:\n")
    ...
    for density, ptype, manufacturer, part in PARTS:
        ...
        sources_to_try = [
            ("DigiKey", lambda p: fetch_digikey_price(p, digikey_token, digikey_id)),
            ("Mouser", lambda p: fetch_mouser_price(p, mouser_key)),
        ]
